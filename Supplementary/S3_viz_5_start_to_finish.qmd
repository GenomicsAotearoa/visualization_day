...
# Start to Finish Visualising your Data

::: {.callout-warning appearance="default"}
# This section has now been retired from the main content. 

Use this material to practise orientating yourself to a new unknown dataset, how to use visualisations to assess and construct your 'mental model' of the data and how to use this to inform your decisions on further analysis of the dataset. 


:::

If you want to start practicing visualisations, a really great place to start is with tidytuesday. Tidytuesday is a community initiative that publishes a weekly dataset in an easy to access format. Every week, people create a visualisation of the data and share it on some form of social media (or just keep it to themselves). In 2024 tidytuesday had the aim of being featured in 10+ training courses. They hit more than 30+!

Check out the [tidytuesday github](https://github.com/rfordatascience/tidytuesday) to see all the data.

In this episode we are going to access some of the tidytuesday data and go through the process of visualising the data. There are hundreds of example datasets available, and some of them are very interesting! Part of the aim of this episode is to explore a dataset that we are not familiar with. This material is based on my first experience participating in tidytuesday, and I had no idea what the data was like or what to expect. 


## Overview

Together, we are going to go from start to finish and create a visualisation. Start to finish means we will need to explore the data, make decisions involving modifications and transformations, and finally, visualise the data in a meaningful way.

*Note: Just the other day I saw a post on LinkedIn, someone had done a screencast recording of them going through a tidytuesday visualisation (in Python!). The poster warns us: "I have to admit, this screencast is 80% me orienting to this week's data". The comments section immediately responds that "80% orienting to the data is real life!"*

### Aims

- To demonstrate the full process of exploratory analysis, data transformation, and visualisation.
...
**Exercise:** Is my hypothesis (very high scores are the result of low sampling bias) true? What type of visualisation would help us to test this? In the space below, create a quick plot to ask whether there is a low-sampling bias present.

```{r}

```

## Assessing data biases and relationships

### Relationship between rating count and rating value
...
## Warning: 29279 rows removed. 
**Missing values or values outside the scale range** - what is this? This warning appears when we attempt to plot values that are outside of the range of the X or Y axis. However, this warning also appears when there is missing data. In this case we know the range of values is 0 - 10, and from the summary() function know that both the Rating variables have 29,279 missing rows. During our initial look at summary(), we noted approx 30,000 missing rows (NAs), leaving approx 30,000 rows of data. If our initial data set didn't have missing values, this warning would be concerning.
...
### Relationship between rating count and release year
...
I have decided to filter perfumes/rows based on a criteria of having been rated 19 or more times (based on the median rating_Count of 19). Perfumes rated fewer than 19 times will be excluded from the analysis. I have also decided to remove brands with only a small number of perfumes (less than 20), as what I am interested in is getting a robust assessment of each brands overall rating, consistent across their perfumes.  

**Exercise:** Discuss with others - do you agree with the decision to filter like this? Can you propose an alternative method? Is our threshold of 19 reasonable? Would you keep all brands regardless of number of perfumes? 
...
While these steps are an important part of visualisation, they are somewhat repetitive and time-consuming and therefore have been compressed and skipped for the sake of this workflow. You can view all of the code and detailed explanations for what steps were taken in the callout box. 
...
*Note:* the thresholds, and the logic behind them, are described in more detail in the callout box below:
...
Sanity checks are an entire arm of bioinformatics and data science, much the same way good visualisation is. It's a skill that takes time and effort to build.
:::
...
With the brandRatingData object stored, we can now proceed to visualisation.
...
In this example we will create a visualisation of the top 20 perfume brands based on the average rating. The **aim** is to provide a novice with information about perfume brands so that they could pick a perfume with some confidence (*e.g.,* if doing online shopping).

It is critically important to identify exactly what you are aiming to represent with your visualisation. Biological data is often complex and, at least when publishing, space is often at a premium and figures tend to be information dense. You must consider the tension between including more information and readability.
...
### Building a visualisation - sketch I

To start a visualisation I will 'sketch' a figure to determine different ways to display the data. A boxplot is a great place to start since I know we are showing average ratings (even though we have so far calculated based on the mean and a boxplot uses medians).
...
**Exercise:** Why is this a poor visualisation, and what can be done to improve it?
...
### Building a visualisation - sketch II
...
This is the basis for a good visualisation.
...
Before we map avg_rating_count to colour we will consider *accessibility*. A colour palette, or visualisation in general, is accessible if it is something that everyone can see. The most regularly discussed examples of accessibility include text size, a colour palette that can be seen by colour-blind individuals, and more recently whether the image is gentle or overwhelming. 
...
# install.packages("ggtext") # (we have commented it out here to prevent this server re-installing the package)
...
This version of the visualisation is:
...
Use the information we have discussed, and the lessons from earlier, to trial out some different geoms and visualisation styles. See what code you can apply (*e.g.,* changing size/colour) with other geoms. 


## Summary

- Before we can visualise our data we may have to transform it in some way or another. We may have to make difficult choices around thresholds of significance or what data to include or remove. 

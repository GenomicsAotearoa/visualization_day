...
This pipe `|>` is actually called the native pipe, and is the one used in R version onwards. You will also see the orignal pipe `%>%` a lot too – they function almost exactly the same, but there are niche scenarios where only one version works. We won't be getting into pipes or tidyverse functions in depth in this workshop. 

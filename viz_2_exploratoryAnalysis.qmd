...
# Taking the jitter further

A commonly used function for jittering is `geom_jitter()`. Unlike geom_quasirandom(), which spaces points based on density, geom_jitter() adds a small amount of random variation to reduce overplotting. While useful for showing individual observations, jitter does not meaningfully represent distribution shape and can sometimes obscure density patterns. For visualising distributions, approaches such as beeswarm, violin, or boxplots are often more appropriate.
...
See here [R documentaion on pheatmap package](https://www.rdocumentation.org/packages/pheatmap/versions/1.0.13/topics/pheatmap). 
...
ggplot(stocks %>% filter(stock == "Stock A"), 
       aes(x = date, y = price)) +
  geom_area(fill = "black", 
            alpha = 0.4) +
  geom_line(color = "black", 
            linewidth = 0.5) +  # Keeps the line for clarity
  labs(title = "Stock A Price Over Time (Area Chart)", 
       x = "Date", 
       y = "Price") +
  theme_minimal()
...
ggplot(stocks %>% filter(stock == "Stock C"), 
  ...
  geom_line(color = "black", 
            linewidth = 0.5) +  # Keeps the line for clarity
  labs(title = "Stock C Price Over Time (Area Chart)", 
       x = "Date", 
       y = "Price") +
  theme_minimal()
```
...
An alternative to stacked area plots is the use of facet, which will allow us to visualise multiple stocks at once by creating individual panels (*e.g.,* try add `+
facet_grid(stock ~ .)` to the stacked area code block earlier. Swapping the tilde, dot and stock order `(~ . stock)` rearranges the layout to vertical instead of horizontal). 
